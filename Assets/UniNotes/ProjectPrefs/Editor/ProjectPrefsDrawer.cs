...
namespace RotaryHeart.Lib.ProjectPreferences
{
    ...
    public class ProjectPrefsDrawer : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            var tempSortBy = (SortBy)EditorGUILayout.EnumPopup(sortBy, EditorStyles.toolbarDropDown, GUILayout.Width(75));
            var tempSort = GUILayout.Toggle(ascending, ascending ? sortAscContent : sortDescContent, EditorStyles.toolbarButton, GUILayout.Width(50));
            ...
            var filterTemp = EditorGUILayout.TextField(filter, EditorStyles.toolbarTextField, GUILayout.Width(position.width - 405));
            ...
            var filterOptionTemp = (SortBy)EditorGUILayout.EnumPopup(filterOption, EditorStyles.toolbarDropDown, GUILayout.Width(75));

            if (filterTemp != null && filter != null && (!filterTemp.Equals(filter) || filterOption != filterOptionTemp))
            {
                filter = filterTemp;
            ...
            GUI.enabled = false;
            string tmp = EditorGUILayout.TextField("Project Prefs Path", PlayerPrefs.GetString("ProjectPrefsPath", System.IO.Path.GetDirectoryName(Application.dataPath + "/../ProjectSettings/") + "/ProjectPreferences.ini"));
            GUI.enabled = true;
            ...
            EditorGUILayout.BeginHorizontal(GUILayout.Width(position.width - 20));
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                data[i].Section = EditorGUILayout.TextField(dataEntry.Section);
                if (string.IsNullOrEmpty(dataEntry.Section))
                {
                    EditorGUI.LabelField(GUILayoutUtility.GetLastRect(), "No Section", EditorStyles.toolbarTextField);
                }
                data[i].Key = EditorGUILayout.TextField(dataEntry.Key);
                data[i].Value = EditorGUILayout.TextField(dataEntry.Value);
                GUI.color = prevColor;

                GUI.color *= 0;
                if (GUILayout.Button(new GUIContent("", "Save"), EditorStyles.toolbarButton, GUILayout.Width(20)))
                {
                ...
                GUI.color *= 0;
                if (GUILayout.Button(new GUIContent("", "Erase"), EditorStyles.toolbarButton, GUILayout.Width(20)))
                {
